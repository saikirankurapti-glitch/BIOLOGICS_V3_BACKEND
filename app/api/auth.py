...
async def get_users():
    ...
    return [
        UserResponse(
            ...
            is_trial_active=getattr(u, 'is_trial_active', True)
        ) for u in users
    ]
...
async def read_users_me(current_user: User = Depends(get_current_user)):
    return UserResponse(
        ...
        tokens=getattr(current_user, 'tokens', 1000),
        trial_ends_at=getattr(current_user, 'trial_ends_at', None),
        is_trial_active=getattr(current_user, 'is_trial_active', True)
    )
...
async def update_user_me(user_update: UserUpdate, current_user: User = Depends(get_current_user)):
    ...
    return UserResponse(
        ...
        tokens=getattr(db_user, 'tokens', 1000),
        trial_ends_at=getattr(db_user, 'trial_ends_at', None),
        is_trial_active=getattr(db_user, 'is_trial_active', True)
    )
