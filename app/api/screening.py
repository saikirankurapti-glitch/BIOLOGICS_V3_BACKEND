...
from app.api.dependencies import get_current_user, require_tokens
...
async def run_screening(
    ...
    current_user: User = Depends(require_tokens(20, "Hit Screening"))
):
...
async def download_screening_report(job_id: str, user: User = Depends(get_current_user)):
    """
    Generate and save a PDF report for a screening job, then download.
    """
    try:
        from bson import ObjectId
        job = await ScreeningJob.get(ObjectId(job_id))
    except:
        job = await ScreeningJob.get(job_id)
    ...
    filename = f"Screening_Report_{str(job.id)[:8]}_{uuid.uuid4().hex[:8]}.pdf"
    ...
    if existing:
        ...
        new_report = ReportRegistry(
            ...
            target_name=job.target_id or "AI Screen",
            report_type="Hit Screening",
            file_path=file_path
        )
    ...
    return Response(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=Screening_Report_{str(job.id)[:8]}.pdf"
        }
    )
