...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Try inserting to MongoDB (fire and forget via task if needed, but we'll await directly here for simplicity)
        try:
            from app.models.log import AccessLog
            # Use asyncio.create_task to avoid blocking the response
            import asyncio
            asyncio.create_task(AccessLog(**log_data).insert())
        except Exception as db_exc:
            error_logger.warning(f"Failed to log to MongoDB: {str(db_exc)}")
